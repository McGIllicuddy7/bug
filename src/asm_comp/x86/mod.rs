...
const FLOAT_ARG_NAMES:&[&'static str] = &["xmm0", "xmm1", "xmm2", "xmm3", "xmm4", "xmm5", "xmm6", "xmm7"];
pub struct ArgCPU{
    int_registers:[u8;6],
...
impl ArgCPU{
    pub fn new()->Self{
        return Self { int_registers: [0,0,0,0,0,0], float_registers: [0,0,0,0,0,0,0,0] };
    }
    ...
    fn generate_basic_arg(&mut self, op_name:&str, size:usize,offset:usize, to_pop_stack:&mut usize)->String{
    ...
    pub fn generate_arg(&mut self, arg_v:&str, arg_t:&Type, to_pop_stack:&mut usize)->String{
        ...
        match arg_t{
            ...
            types::Type::StructT { name:_, components }=>{
                let mut offset = 0;
                for i in components{
